...
export function buildArticlePrompt(): string {
...
- Business/government pricing is never stated. Residential flat rates may be referenced exactly: ${residentialServices.map((s) => `${s.name} ${s.priceDisplay}`).join("; ")}.
